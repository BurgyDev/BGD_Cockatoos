local mainMenu = RageUI.CreateMenu("Bar VIP", "Bar VIP")
			...
			while open do
                ...
                RageUI.IsVisible(mainMenu, function()
                    if not craftencours then
                        for k,v in pairs(BGD_Cockatoos.BarVIP1.List) do
...
Citizen.CreateThread(function()
    while true do
    	local wait = 900
    	for k,v in pairs(BGD_Cockatoos.BarVIP1.Pos) do
        	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'cockatoos' then
		        ...
		        wait = 0
            		...
            		if dist <= 1.0 then 
						wait = 0
						ESX.ShowHelpNotification("~INPUT_TALK~ pour ouvrir le Bar VIP")
